...
/**
 * Default layout configuration for the margin trading page
 */
export const getMarginLayout = (WIDGET_REGISTRY: Record<string, any>): LayoutWidget[] => [
  { 
    x: 0, 
    y: 0, 
    w: Math.max(8, WIDGET_REGISTRY['trading-view-chart'].minSize.w), 
  ...
  { 
    x: 8, 
    y: 6, 
    w: Math.max(4, WIDGET_REGISTRY['recent-trades'].minSize.w), 
    h: Math.max(4, WIDGET_REGISTRY['recent-trades'].minSize.h), 
    id: 'trades', 
